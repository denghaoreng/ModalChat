...
| 9 | [常见Bug与解决方案](../bug/00-常见Bug与解决方案.md) | 17个典型Bug的根因分析、解决方案、通用建议 |

## 🔧 实战经验总结

本指南基于多个 SillyTavern 插件的实际开发经验，帮助你避免常见陷阱：
...
- **模块拆分**（见 `01-插件开发总览.md`）：复杂插件应拆分为独立模块
- **避免 html() 覆盖兄弟容器**：将独立组件放在平级位置，不嵌套在会被覆盖的容器内
- **批量操作合并 saveSettings**：批量 CRUD 应直接操作数据数组，最后单次保存
